...
import java.sql.*;
import java.io.*;
public class RmiReadServer3 extends
 ...
 // RMIFromServer rmiserver2;
  ...
  public String receiveVersion1(String x,String y) throws RemoteException,SQLException
  {
      ...
      PreparedStatement ps=con.prepareStatement("select version_no from student_records3 where st_id=?");
      ps.setString(1,x);
  ...
  public String selectrecords(String x) throws RemoteException,SQLException
{
    ...
    PreparedStatement ps=con.prepareStatement("select * from student_records3 where st_id=?");
    ps.setString(1, x);
    ResultSet rs=ps.executeQuery();
    while(rs.next())
    {
  ...
  public void display(String[] ar)throws RemoteException
{
   ...
   public int updaterecords(String rno,String[] array)throws RemoteException,SQLException
  {
    ...
    System.out.println("Record To update : "+array[i]);
          ...
          Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
          Connection con=DriverManager.getConnection("jdbc:odbc:quorumdsn","usr","pwd");
           PreparedStatement ps=con.prepareStatement("update student_records3 set version_no=?, st_name=?, st_address=?,phone_no=? where st_id=?");
        //  System.out.println("version no array2"+a);
          ...
          int p=ps.executeUpdate();
           System.out.println("Table3 updated");
          if(p==1)
              return 1;
          else
              return 0;
...
public int writenewrecord(int vno,String sid, String sname, String saddress, int phone) throws RemoteException,SQLException
     ...
     try
      {
           Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
          Connection con=DriverManager.getConnection("jdbc:odbc:quorumdsn","usr","pwd");
          PreparedStatement ps=con.prepareStatement("insert into student_records3 values(?,?,?,?,?)");
          ps.setInt(1,vno);
          ps.setString(2,sid);
  ...
  public RmiReadServer3() throws RemoteException,SQLException{
  ...
  registry.rebind("rmiServer3", this);
...
registry.rebind("rminewwrite",this);
  ...
  RmiReadServer3 server = new RmiReadServer3();
