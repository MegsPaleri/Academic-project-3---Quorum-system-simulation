...
import java.sql.*;
import java.io.*;
public class RmiReadServer5 extends
 ...
 // RMIFromServer rmiserver2;
  ...
  public String receiveVersion1(String x,String y) throws RemoteException,SQLException
  {
      ...
      PreparedStatement ps=con.prepareStatement("select version_no from student_records5 where st_id=?");
      ps.setString(1,x);
 ...
 public String selectrecords(String x) throws RemoteException,SQLException
{
    ...
    PreparedStatement ps=con.prepareStatement("select * from student_records5 where st_id=?");
    ps.setString(1, x);
    ResultSet rs=ps.executeQuery();
    while(rs.next())
    {
 ...
 public void display(String[] ar)throws RemoteException
{
   ...
   public int updaterecords(String rno,String[] array)throws RemoteException,SQLException
  {
      try
      {
          for(int i=0;i<5;i++)
    {
    System.out.println("Records to update: "+array[i]);
    }
       
          Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
          Connection con=DriverManager.getConnection("jdbc:odbc:quorumdsn","usr","pwd");
           PreparedStatement ps=con.prepareStatement("update student_records5 set version_no=?, st_name=?, st_address=?,phone_no=? where st_id=?");
     ...
     public int writenewrecord(int vno,String sid, String sname, String saddress, int phone) throws RemoteException,SQLException
  {
      try
      {
           Class.forName("sun.jdbc.odbc.JdbcOdbcDriver");
          Connection con=DriverManager.getConnection("jdbc:odbc:quorumdsn","usr","pwd");
          PreparedStatement ps=con.prepareStatement("insert into student_records5 values(?,?,?,?,?)");
          ps.setInt(1,vno);
          ps.setString(2,sid);
  ...
  public RmiReadServer5() throws RemoteException,SQLException{
  ...
  registry.rebind("rmiServer5", this);
  ...
  RmiReadServer5 server = new RmiReadServer5();
